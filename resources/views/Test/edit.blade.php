            ...
            <div class="col-lg-6 mx-auto">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h4 class="card-title mb-0 flex-grow-1">Update Test Schedule</h4>
                    </div>
                    <div class="card-body">
                        <div class="live-preview">
                            <form id="form_update" class="row g-3 needs-validation" novalidate>
                                    ...
                                    <select class="single-select" name="degree" id="degree_getting" autocomplete="off" required>
                                        ...
                                        @foreach ($alldegree as $degree)
                                            <option value="{{ $degree->id }}"
                                                {{ $test->degree == $degree->id ? 'selected' : '' }}>
                                    ...
                                    <select class="single-select" name="class" id="subject_getting" autocomplete="off" required>
                                        ...
                                        @foreach ($array_subjects as $subjects)
                                            <option value="{{ $subjects }}"
                                                {{ $test->class == $subjects ? 'selected' : '' }}>{{ $subjects }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="date" name="date" value="{{ $test->date }}" class="form-control" required>
                                ...
                                <div class="row mt-3">
                                    ...
                                    <div class="col-6 text-end">
                                        <button class="btn rounded-pill btn-primary waves-effect waves-light"
                                            type="submit" id="update">Update Schedule > </button>
                                    </div>
                                </div>
                            </form>
                        </div>
