            ...
            <div class="col-lg-9 mx-auto">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h4 class="card-title mb-0 flex-grow-1">Update Books</h4>
                    </div>
                    <div class="card-body">
                        <div class="live-preview">
                            <form id="form_update" class="row g-3 needs-validation" novalidate>
                                    ...
                                    <select class="single-select" name="degree" id="degree" autocomplete="off" required>
                                        ...
                                        @foreach ($alldegree as $degree)
                                            <option value="{{ $degree->id }}"
                                                {{ $book->degree == $degree->id ? 'selected' : '' }}>
                                    ...
                                    <select class="single-select" name="class" autocomplete="off" required>
                                        ...
                                        @foreach ($allclasses as $classes)
                                            <option value="{{ $classes->id }}"
                                                {{ $book->class == $classes->id ? 'selected' : '' }}>
                                ...
                                @php
                                    $loopcount = 1;
                                    $all = unserialize($book->subject);
                                @endphp
                                @foreach ($all as $key => $ali)
                                    @php
                                        $loopprevious = $loopcount;
                                    @endphp
                                    <div id="product_fields">
                                        <div id="product_attr_{{ $loopcount++ }}">

                                            <div class="row g-3 mt-0">
                                                <div class="col-md-4">
                                                    @php
                                                        $sub = unserialize($book->subject);
                                                    @endphp
                                                    <label for="Subject" class="form-label">Subject</label>
                                                    <input type="text" class="form-control" value="{{ $sub[$key] }}"
                                                        id="Subject" name="subject[]" oninput="allow_alphabets(this)"
                                                ...
                                                <div class="col-md-4">
                                                    ...
                                                    @else
                                                        <button type="button" onclick="remove_more({{ $loopprevious }});"
                                                            class=" mt-4 btn rounded-pill px-4 btn-outline-danger font-medium waves-effect waves-light"><i
...
@section('script')
    ...
    <script>
        ...
        function add_more() {
            ...
            html +=
                '<div class="col-md-4"><label for="Subject" class="form-label">Subject</label><input type="text" class="form-control" id="Subject" name="subject[]" oninput="allow_alphabets(this)" placeholder="Enter Subject Name" required></div>';

            html += '<div class="col-md-4"><button type="button" onclick="remove_more(' + count +
